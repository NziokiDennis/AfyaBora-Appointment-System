<?php
// Expects $current_page to be set by the including page (e.g. 'dashboard', 'book_appointment', 'medical_history', 'feedback').
// Expects $conn (mysqli) and a logged-in patient session to already exist.

$__sidebar_user_id = $_SESSION["user_id"] ?? null;
$__profile_pct = 0;
$__profile_checks = [];
...
if ($__sidebar_user_id) {
    ...
    $__profile_checks = [
        "Add phone number"     => !empty($__prof["phone_number"]),
        "Add date of birth"    => !empty($__prof["date_of_birth"]),
        "Add gender"           => !empty($__prof["gender"]),
        "Add address"          => !empty($__prof["address"]),
    ];
    $__filled = count(array_filter($__profile_checks));
    $__total  = count($__profile_checks);
    $__profile_pct = $__total ? (int)round(($__filled / $__total) * 100) : 0;
}
?>
<style>
:root {
  /* Exact values pixel-sampled from the Medora Health reference dashboard. */
  --navy: #002d70;
  --navy2: #134589;
  --blue: #0b63c3;
  --blue2: #094f9e;
...
.ab-nav-item i { width: 18px; text-align: center; font-size: .9rem; }
...
.aw-check i { font-size: .8rem; width: 14px; }
.aw-check.done i { color: var(--green); }
.aw-check.pending i { color: rgba(255,255,255,.3); }
.aw-check.done { color: rgba(255,255,255,.5); text-decoration: line-through; }
...
.ab-topbar-left .ab-subgreeting { font-size: .8rem; color: var(--muted); margin-top: 2px; }
...
.ab-btn-sm { padding: 6px 12px; font-size: .78rem; }
...
.ab-user-name { font-size: .85rem; font-weight: 600; color: var(--navy); }

.ab-content { padding: 24px 28px 40px; flex: 1; }

/* Centers a lone card on the ACTUAL viewport, not just within the content
   strip beside the sidebar -- centering with plain margin:auto inside
   .ab-content only balances the space *after* the sidebar, which visually
   sits right-of-center on the real screen. Apply to any single centered
   card (e.g. a lone form) via class="ab-card ab-center-viewport". */
...
.ab-page-sub { color: var(--muted); font-size: .88rem; margin-bottom: 22px; }

/* Shared list-row: icon chip -> title line (+ inline tags/pills) -> meta line -> optional detail lines -> trailing action(s). */
...
.ab-list-row .alr-title { font-weight: 600; color: var(--navy); font-size: .9rem; }
.ab-list-row .alr-meta { color: var(--muted); font-size: .76rem; margin-top: 3px; }
.ab-list-row .alr-detail { font-size: .8rem; color: var(--navy); margin-top: 8px; }
...
.ab-list-row .alr-view { color: var(--border); font-size: .8rem; transition: color .15s; }
.ab-list-row .alr-view:hover { color: var(--blue); }

.ab-alert { padding: 11px 15px; border-radius: var(--radius-sm); font-size: .85rem; margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
...
/* Form controls: shared across every AfyaBora form (register, booking, profile, etc.) */
...
.ab-label { display: block; font-size: .78rem; font-weight: 600; color: var(--navy); margin-bottom: 7px; }
...
.ab-field-hint { font-size: .76rem; color: var(--muted); margin-top: 6px; }
.ab-field-error { font-size: .78rem; color: var(--rose); margin-top: 6px; display: flex; align-items: center; gap: 6px; }
.ab-field-error[hidden] { display: none; }
.ab-info-banner {
  background: var(--sky); border: 1px solid var(--border); border-radius: var(--radius-md);
  padding: 12px 14px; margin-bottom: 18px; font-size: .82rem; color: var(--navy); line-height: 1.5;
}
...
<aside class="ab-sidebar">
  ...
  <nav class="ab-nav">
    ...
    <a href="book_appointment.php" class="ab-nav-item <?= $current_page === 'book_appointment' ? 'active' : '' ?>"><i class="fas fa-calendar-plus"></i> Book Appointment</a>
    <a href="medical_history.php" class="ab-nav-item <?= $current_page === 'medical_history' ? 'active' : '' ?>"><i class="fas fa-notes-medical"></i> Medical History</a>
    <a href="feedback.php" class="ab-nav-item <?= $current_page === 'feedback' ? 'active' : '' ?>"><i class="fas fa-comment-dots"></i> Feedback</a>
    <a href="update_profile.php" class="ab-nav-item <?= $current_page === 'update_profile' ? 'active' : '' ?>"><i class="fas fa-user-pen"></i> Update Profile</a>
    <a href="../logout.php" class="ab-nav-item logout"><i class="fas fa-sign-out-alt"></i> Sign Out</a>
  </nav>

  <?php if ($__sidebar_user_id): ?>
  <div class="ab-sidebar-widget">
    <div class="aw-title">Profile Completion</div>
    <div class="aw-pct"><?= $__profile_pct ?>%</div>
    <div class="aw-bar"><div class="aw-bar-fill" style="width: <?= $__profile_pct ?>%"></div></div>
    <?php foreach ($__profile_checks as $__label => $__done): ?>
      <div class="aw-check <?= $__done ? 'done' : 'pending' ?>">
        <i class="fas <?= $__done ? 'fa-circle-check' : 'fa-circle' ?>"></i> <?= htmlspecialchars($__label) ?>
      </div>
    <?php endforeach; ?>
  </div>
  <?php endif; ?>
