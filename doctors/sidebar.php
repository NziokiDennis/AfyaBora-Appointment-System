<?php
// Expects $current_page to be set by the including page (e.g. 'dashboard', 'appointments', 'schedule', 'medical_history', 'feedback').
// Expects $conn (mysqli) and a logged-in doctor session to already exist.

$__sidebar_doctor_id = $_SESSION["user_id"] ?? null;
$__today_count = 0;
$__specialization = null;

if ($__sidebar_doctor_id) {
    $__stmt = $conn->prepare("SELECT COUNT(*) AS c FROM appointments WHERE doctor_id = ? AND appointment_date = CURDATE() AND status = 'scheduled'");
    $__stmt->bind_param("i", $__sidebar_doctor_id);
    $__stmt->execute();
    $__today_count = (int)($__stmt->get_result()->fetch_assoc()["c"] ?? 0);

    $__spec_stmt = $conn->prepare("SELECT specialization FROM users WHERE user_id = ?");
    $__spec_stmt->bind_param("i", $__sidebar_doctor_id);
    $__spec_stmt->execute();
    $__specialization = $__spec_stmt->get_result()->fetch_assoc()["specialization"] ?? null;
}
?>
<style>
:root {
...
.ab-sidebar-widget .aw-spec strong { display: block; color: #fff; font-size: .85rem; margin-top: 2px; }
...
<aside class="ab-sidebar">
  ...
  <nav class="ab-nav">
    ...
    <a href="appointments.php" class="ab-nav-item <?= $current_page === 'appointments' ? 'active' : '' ?>"><i class="fas fa-calendar-check"></i> Appointments</a>
    <a href="schedule.php" class="ab-nav-item <?= $current_page === 'schedule' ? 'active' : '' ?>"><i class="fas fa-clock"></i> My Schedule</a>
    <a href="medical_history.php" class="ab-nav-item <?= $current_page === 'medical_history' ? 'active' : '' ?>"><i class="fas fa-notes-medical"></i> Medical Records</a>
    <a href="view_feedback.php" class="ab-nav-item <?= $current_page === 'view_feedback' ? 'active' : '' ?>"><i class="fas fa-star"></i> Feedback</a>
    <a href="../logout.php" class="ab-nav-item logout"><i class="fas fa-sign-out-alt"></i> Sign Out</a>
  </nav>

  <?php if ($__sidebar_doctor_id): ?>
  <div class="ab-sidebar-widget">
    <div class="aw-title">Today's Appointments</div>
    <div class="aw-pct"><?= $__today_count ?></div>
    <div class="aw-note"><?= $__today_count === 1 ? 'appointment scheduled today' : 'appointments scheduled today' ?></div>
    <?php if ($__specialization): ?>
    <div class="aw-spec">
      Specialization
      <strong><?= htmlspecialchars($__specialization) ?></strong>
    </div>
    <?php endif; ?>
  </div>
  <?php endif; ?>
